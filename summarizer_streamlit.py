...
def create_safe_summarizer():
    ...
    """
    with st.spinner("Initializing BART-large-CNN summarization model..."):
...
def extract_text_and_metadata_from_pdf(pdf_path, verbose=True):
    ...
    try:
        ...
        # Create a progress bar for page extraction
        progress_bar = st.progress(0)
        
        for page_num, page in enumerate(doc, 1):
            if verbose and page_num % 5 == 0:  # Update progress every 5 pages
...
def summarize_text_safely(summarizer, text, verbose=False, retry_count=0):
    ...
    if verbose:
        st.info(f"Summarizing text segment ({len(text)} characters)...")
    ...
    if len(cleaned_text) < 0.7 * len(text):
        st.warning(f"Text was significantly cleaned (from {len(text)} to {len(cleaned_text)} chars)")
    ...
    try:
        ...
        if len(cleaned_text) > 1000:
            ...
            if verbose:
                st.info(f"First attempting with truncated text ({len(try_text)} chars)")
            ...
            if summary and len(summary) > 0 and len(summary[0]['summary_text']) > 30:
                ...
                if verbose:
                    st.info("Short text summarization failed, trying with more content")
        ...
        if summary and len(summary) > 0:
            ...
            if verbose:
                st.warning("Summarizer returned empty result")
            ...
            if len(cleaned_text) > 500 and retry_count == 0:
                ...
                if chunks:
                    ...
                    for i, chunk in enumerate(chunks[:3]):  # Limit to first 3 chunks
                        if verbose:
                            st.info(f"Summarizing chunk {i+1}/{min(len(chunks), 3)}")
        ...
        if len(cleaned_text) > 500:
            ...
            try:
                if verbose:
                    st.info("Attempting fallback with shorter text...")
                ...
                for size in [800, 500, 300]:
                    if len(cleaned_text) > size:
                        ...
                        try:
                            ...
                            if verbose:
                                st.info(f"Fallback with {size} chars failed, trying smaller...")
            except Exception as inner_e:
                st.error(f"All fallbacks failed: {inner_e}")
...
def summarize_pdf(pdf_path, verbose=True):
    ...
    try:
        ...
        if sections and len(sections) >= 2:  # At least 2 sections to be considered structured
            if verbose:
                st.info(f"Found {len(sections)} document sections")
            ...
            if 'abstract' in sections and len(sections['abstract']) > 100:
                if verbose:
                    st.info(f"Processing ABSTRACT section... ({len(sections['abstract'])} chars)")
            ...
            for section_name in section_order:
                if section_name in sections:
                    ...
                    if verbose:
                        st.info(f"Summarizing {section_name.upper()} section... ({len(section_text)} chars)")
            ...
            if not section_summaries_added:
                if verbose:
                    st.warning("Section summarization produced no results, falling back to whole document summarization.")
            ...
            if verbose:
                st.info("No clear sections found. Summarizing document as a whole.")
            perform_whole_document_summarization = True
        ...
        if perform_whole_document_summarization:
            ...
            if 'abstract' in sections and len(sections['abstract']) > 200:
                if verbose:
                    st.info("Using abstract as main summary source")
            ...
            if verbose:
                st.info(f"Summarizing document introduction ({len(intro_text)} chars)")
            ...
            for i, chunk in enumerate(intro_chunks):
                if verbose:
                    st.info(f"  Processing intro chunk {i+1}/{len(intro_chunks)}")
            ...
            if verbose:
                st.info(f"Summarizing main document content ({len(full_text)} chars)")
            ...
            if len(chunks) > 10:
                if verbose:
                    st.info(f"Document is very long ({len(chunks)} chunks). Sampling throughout.")
            ...
            for i, chunk in enumerate(chunks):
                if verbose:
                    st.info(f"  Processing main content chunk {i+1}/{len(chunks)}")
        ...
        if len(summary_parts) <= 6:  # Only metadata and separator were added
            st.warning("Failed to generate summary content through normal means.")
            ...
            try:
                ...
                st.error(f"Fallback summarization also failed: {fallback_error}")
...
def create_qa_system():
    ...
    """
    with st.spinner("Initializing question-answering model..."):
        try:
...
def ask_question(question, summary, full_text, metadata, verbose=False):
    ...
    try:
        ...
        if verbose:
            st.info(f"Processing content question: {question}")
            if not is_metadata_question:
                st.info("This is NOT a metadata question - searching document content")
        ...
        if verbose:
            st.info(f"Using {len(relevant_text)} chars of relevant text for question answering")
        ...
        with st.spinner("Finding answer..."):
            ...
            if answer['score'] < 0.1 or len(answer['answer'].strip()) < 5:
                ...
                if verbose:
                    st.warning(f"Low confidence answer (score: {answer['score']:.2f}), trying alternative approach")
...
def main():
    ...
    st.title("📑 PDF Summarizer & Question Answering Tool")
    st.markdown("""
    This application analyzes academic papers and research documents, generating
    comprehensive summaries and allowing you to ask questions about the content.
    ...
    It extracts metadata, identifies document sections, and generates a structured summary.
    """)
    ...
    if uploaded_file is not None:
        if not st.session_state.file_processed or st.button("Re-process PDF"):
            st.session_state.file_processed = False
            st.session_state.qa_history = []  # Clear QA history for new file
            ...
            with st.spinner("Processing PDF... This may take a few minutes depending on the file size."):
                ...
                st.session_state.summary, st.session_state.full_text, st.session_state.metadata = summarize_pdf(pdf_path, verbose=True)
        ...
        # Display summary and metadata if processing is complete
        if st.session_state.file_processed:
            # Display document info and summary
            st.header("Document Summary")
            
            # Create two columns - one for metadata, one for summary
            col1, col2 = st.columns([1, 3])
            
            with col1:
                st.subheader("Document Information")
                st.write(f"**Title:** {st.session_state.metadata['title']}")
                if st.session_state.metadata['authors']:
                    st.write(f"**Authors:** {st.session_state.metadata['authors']}")
                st.write(f"**Pages:** {st.session_state.metadata['pages']}")
                if st.session_state.metadata['publication_date']:
                    st.write(f"**Publication Year:** {st.session_state.metadata['publication_date']}")
                
                # Download buttons
                st.download_button(
                    label="Download Summary",
                    data=st.session_state.summary,
                    file_name=f"{st.session_state.metadata['title']}_summary.txt",
                    mime="text/plain"
                )
                st.download_button(
                    label="Download Full Text",
                    data=st.session_state.full_text,
                    file_name=f"{st.session_state.metadata['title']}_full_text.txt",
                    mime="text/plain"
                )
            ...
            st.markdown("""
            You can ask questions about the document content, such as:
            ...
            if st.button("Ask") and question:
                with st.spinner("Finding the answer..."):
                    answer = ask_question(question, st.session_state.summary, 
            ...
            if st.session_state.qa_history:
                ...
                for i, (q, a) in enumerate(st.session_state.qa_history):
                    with st.expander(f"Q: {q}", expanded=(i == len(st.session_state.qa_history) - 1)):
        ...
        - Extract and organize key information from research papers
        - Identify document sections (abstract, introduction, methods, results, etc.)
        - Generate concise, structured summaries
        
        #### Question Answering
        - Answer questions about the document content
        - Extract specific information from lengthy papers
